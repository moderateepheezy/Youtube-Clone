...
class VideoCell: BaseCell{
    ...
    var video: Video? {
        didSet{
            ...
            if let channelName = video?.channel?.name, let numberOfViews = video?.number_of_views{
                ...
                let subtitleText = "\(channelName) • \(numberOfFormatter.string(from: numberOfViews)!) •  2 years ago"
    ...
    func setupProfileImage(){
        if let profImgUrl = video?.channel?.profile_image_name{
            userProfileImageView.loadImageUsingUrlString(urlString: profImgUrl)
        }
    }
    
    let thumbnailImageView: CustomImageView = {
        let imageView = CustomImageView()
        imageView.image = UIImage(named: "tailor_swift_back")
    ...
    let userProfileImageView: CustomImageView = {
        let imageView = CustomImageView()
        imageView.image = UIImage(named: "tailor_circular")
